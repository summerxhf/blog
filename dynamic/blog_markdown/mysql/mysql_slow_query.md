...
18、mysql union 操作符,将多个select 操作结果集合中,并且会删除重复数据，就不需要使用DISTINCT了。而union all 会有重复的值。<br>

19、注意mysql group by语法，group by语句时根据一个或者多个列结果集进行分组。在分组的列上可以使用count、sum、avg等函数，注意不要选择无关的列多余列，会导致数据错误<br>
- 优化示例<br>
